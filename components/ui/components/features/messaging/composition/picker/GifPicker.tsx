...
type GifCategory = { query: string; preview_url: string };
...
function Categories() {
  ...
  const trendingCategories = useQuery<GifCategory[]>(() => ({
    ...
    queryFn: () => {
      return fetch(`${env.DEFAULT_GIFBOX_URL}/categories?locale=en_US`).then((r) => r.json()).then((resp) => {
  ...
  const trendingGif = useQuery<GifResult | null>(() => ({
    ...
    queryFn: () => {
      return fetch(`${env.DEFAULT_GIFBOX_URL}/trending?locale=en_US&limit=1`, {
        ...
        .then((r) => r.json())
        .then((resp) => {
          if (!resp.data?.data?.[0]) return null;
          const item = resp.data.data[0];
          return {
            url: item.file.hd.gif.url,
            media_formats: {
              webm: { url: item.file.hd.webm.url },
              tinywebm: { url: item.file.md.webm.url }
            },
          };
        });
    },
  ...
  return (
    <Category
      style={{
        ...
        "background-image": `linear-gradient(to right, #0006, #0006), url("${props.item.t === 0 ? props.item.category.preview_url : props.item.gif?.url}")`,
      }}
      tabIndex={props.tabIndex}
      role="listitem"
      onClick={() =>
        setFilter!(props.item.t === 0 ? props.item.category.query : "trending")
      }
      onMouseDown={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    >
      <Switch fallback={<Trans>Trending GIFs</Trans>}>
        <Match when={props.item.t === 0}>
          {(props.item as CategoryItem & { t: 0 }).category.query}
        </Match>
      </Switch>
    </Category>
  );
      ...
      return fetch(
        `${env.DEFAULT_GIFBOX_URL}/` +
          (props.query === "trending"
            ...
            : `search?locale=en_US&q=${encodeURIComponent(props.query)}`),
        {
          headers: {
